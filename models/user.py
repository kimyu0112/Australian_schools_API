...
class UserSchema(ma.Schema):    
    ...
    password = fields.String(required=True, validate=Regexp("^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$", error="Minimum eight characters, at least one letter, one number and one special character"))
