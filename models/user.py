...
class UserSchema(ma.Schema):    
    ...
    password = fields.String(required=True, validate=Regexp("^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$", error="password should contain minimum eight characters with at least one letter, one number and one special character."))
